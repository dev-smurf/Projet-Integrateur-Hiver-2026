<template>
  <div class="max-w-3xl">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">{{ $t('routes.books.children.add.name') }}</h1>
...
import type {ICreateBookRequest} from "@/types/requests/createBookRequest";
...
const form = reactive<ICreateBookRequest>({});
...
async function handleSubmit() {
  ...
  const response = await bookService.createBook(formData as any);
  if (response.succeeded) {
    notify({type: "success", text: t("validation.book.add.success")});
    await router.push({name: "books.index"});
  } else {
    apiErrors.value = response.getErrorMessages("validation.book.add");
  }

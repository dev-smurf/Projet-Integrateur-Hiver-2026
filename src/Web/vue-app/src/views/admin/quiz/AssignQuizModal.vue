  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
    <div class="flex max-h-[760px] w-full max-w-3xl flex-col overflow-hidden rounded-lg bg-white shadow-lg">
        ...
        <template v-else>
          <div class="space-y-3 rounded-lg border border-gray-200 p-4">
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
                ...
                <div v-else class="max-h-72 space-y-2 overflow-y-auto pr-1">
                  ...
                  >
                    ...
                    <input
                      ...
                      :disabled="eligibleEquipeUserIds(equipeIdOf(equipe)).length === 0"
                      class="h-4 w-4 rounded text-blue-600 disabled:opacity-40"
                    />
                  </label>
                </div>
              </div>
              ...
              <div class="space-y-3">
                <span class="block text-sm font-medium text-gray-700">{{ listTitle }}</span>
                <input
                  v-model="searchQuery"
                  type="text"
      ...
      <div class="flex justify-end gap-3 border-t border-gray-200 bg-gray-50 p-4">
        ...
        <button
          @click="handleAssign"
          :disabled="selectedAssignmentUserIds.length === 0 || assigning"
...
import { useEquipesService, useMemberService, useQuizService } from '@/inversify.config'
...
const assignedUserIdSet = computed(() => new Set(assignedUserIds.value))
...
watch(mode, () => {
  ...
  selectedEquipeIds.value = []
  searchQuery.value = ''
  equipeSearchQuery.value = ''
})
...
async function handleAssign() {
  ...
  try {
    ...
    await quizService.assignQuiz(props.quizId, selectedAssignmentUserIds.value, label, availableDate, due)
...
function eligibleEquipeUserIds(equipeId: string): string[] {
  ...
  return ids.filter(userId => mode.value === 'initial'
    ? !assignedUserIdSet.value.has(userId)
    : assignedUserIdSet.value.has(userId))
}
    ...
    try {
      ...
      const memberEntries = await Promise.all(equipes.value.map(async equipe => {
        ...
        const userIds = (response?.members || [])
          .map((member: any) => member.userId ?? member.UserId)
          .filter(Boolean)
        return [equipeId, userIds] as const
      }))
      equipeMembersByEquipeId.value = Object.fromEntries(memberEntries)
    } catch (err) {
      console.warn('Failed to load equipes:', err)
      equipes.value = []
      equipeMembersByEquipeId.value = {}
    }
  } catch (err) {

...
onMounted(async () => {
  try {
    ...
    if (!mod) {
      loadError.value = "Module not found.";
      loading.value = false;
      return;
    }
    formData.value = {
...
async function handleSubmit() {
  ...
  try {
    const result = await modulesService.updateModule(formData.value.id || props.id, formData.value);
    if (result.succeeded) {
      notify({type: "success", text: "Module updated."});
      await router.push({name: "admin.children.modules.index"});
    } else {
      notify({type: "error", text: "Error updating module."});
    }
  } catch {
    notify({type: "error", text: "Error updating module."});
  }

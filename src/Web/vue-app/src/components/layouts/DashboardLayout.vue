...
import Cookies from "universal-cookie";
    import { LayoutDashboard, BookOpen, Shield, LogOut, Languages, CheckCircle2, XCircle, X, Users, Layers, UsersRound, ClipboardCheck } from "lucide-vue-next";
...
<template>
    <div class="min-h-screen bg-gray-100">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <!-- Bottom: language + profile + logout -->
        ...
        <div class="flex-1 flex flex-col min-h-screen min-w-0 lg:ml-60">
            <!-- Top bar -->
            <header class="sticky top-0 z-30 bg-white border-b border-gray-200 px-6 h-14 flex items-center justify-between lg:justify-end">
                <button @click="sidebarOpen = true"
                        class="p-2 -ml-2 text-gray-500 hover:text-gray-700 rounded-lg lg:hidden cursor-pointer">
                    <Menu class="w-5 h-5" />
                </button>

                <div class="flex items-center gap-3">
                    <router-link :to="{ name: 'account' }"
                                 class="w-8 h-8 rounded-full bg-brand-600 text-white flex items-center justify-center text-xs font-semibold">
                        {{ initials }}
                    </router-link>
                </div>
            </header>
